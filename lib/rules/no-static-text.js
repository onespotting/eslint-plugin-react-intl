...
function generateId(context, prefix) {
  let id = "";
  const topFileName = basename(context.getFilename(), extname(context.getFilename()));
  id = id + (dirname(context.getFilename()) + '.' + topFileName).replaceAll('/', '.') + '.';
  id = id.replace(/[A-Z]/g, m => "_" + m.toLowerCase()).replaceAll('._', '.').replaceAll('-', '_').replace(/^.+\.src\./, '');
  id = prefix + id;
  return id;
}
    ...
    function reportLiteralNode(node, messageId) {
      ...
      report(context, messages[messageId], messageId, {
        ...
        fix: function (fixer) {
          ...
          let id = context.getFilename() ? generateId(context, "app.") : "";
          return fixer.replaceText(node, "<FormattedMessage id=\"" + id + "\" defaultMessage=\"" + text + "\"/>");
        }
      });
    }
